...
const DiarioDownload = ({ diario, token }) => {
  ...
  const getFileFromServer = async (extensao) => {
    ...
    if (extensao === 'csv') {
      arquivoDownload = `${arquivo}-${ano}${mes}${dia}`
      MIME_TYPE = 'text/csv'
      config = {
        headers: {
          'Content-Type': 'application/json',
    ...
    const postDiario = { arquivo: arquivoDownload, extensao }
    // console.log('postData', postData)

    try {
      const res = await axios.post(url, postDiario, config)
      let fileBlob = ''
      if (extensao === 'json') {
        const { data } = res
        // console.log('res : ', res)
...
DiarioDownload.propTypes = {
  diario: PropTypes.shape({
    uf: PropTypes.string,
    ano: PropTypes.string,
    mes: PropTypes.string,
    dia: PropTypes.string,
    gname: PropTypes.string,
    diario: PropTypes.string
  }).isRequired,
